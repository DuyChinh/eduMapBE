...
const generateResponseFile = async (prompt, attachments = [], history = []) => {
    try {
        let contents = [];
...
const generateResponse = async (prompt, attachments = [], history = []) => {
    // Redirect to the robust implementation that handles IPv4 enforcement and retries
    return generateResponseFile(prompt, attachments, history);
};
